pipeline {
    ...
    stages {
       ...
       stage('Build') {
           steps {
               script {
                   ...
                   sh """
                   cat "${logFile.getPath()}" | perl -pe '\x1b\\[8m.*?\\x1b\\[0m//g;' > build-log.txt 2>&1
                   """
       ...
       stage('Terminate Legion Airflow') {
           steps {
               script {
                   result = build job: env.param_terminate_legioin_airflow_job_name, propagate: true, wait: true, parameters: [
                           [$class: 'GitParameterValue', name: 'GitBranch', value: env.mergeBranch],
                           string(name: 'Profile', value: env.param_profile),
                           string(name: 'LegionVersion', value: legionVersion),
                           string(name: 'EnclaveName', value: 'company-a')
